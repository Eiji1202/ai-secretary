...
宿検索の結果を返すときは、各宿のリンク（🔗と📋）を必ずそのまま含めること。省略しないこと。
...
const tools = [
  ...
  {
    name: 'search_hotel',
    description: '楽天トラベルで空室のある宿を検索する',
    input_schema: {
      type: 'object',
      properties: {
        checkinDate: { type: 'string', description: 'チェックイン日（YYYY-MM-DD）' },
        checkoutDate: { type: 'string', description: 'チェックアウト日（YYYY-MM-DD）' },
        adultNum: { type: 'number', description: '大人の人数' },
        maxCharge: { type: 'number', description: '1人1泊の上限金額' },
        mealClass: { type: 'number', description: '食事: 1=朝食付き 2=夕食付き 3=2食付き' },
        keyword: { type: 'string', description: '検索キーワード（例：ペット可、露天風呂、旅館、高級、リゾート）' },
        prefectures: { type: 'array', items: { type: 'string' }, description: '都道府県名の配列（例：["岐阜", "三重", "静岡"]）。県は「〜県」を除いた名前で指定' },
        squeezeCondition: { type: 'string', description: '絞り込み条件をカンマ区切りで指定。onsen=温泉, kinen=禁煙, daiyokujou=大浴場, wifi=Wi-Fi' },
        sort: { type: 'string', description: 'ソート順。standard=おすすめ順(デフォルト), +roomCharge=安い順, -roomCharge=高い順' },
        hits: { type: 'number', description: '取得件数（デフォルト5件、複数県の場合は合計件数）' },
      },
...
async function executeTool(
  ...
  switch (toolName) {
    ...
    case 'search_hotel': {
      const { searchHotels } = await import('./hotel')
      const hotels = await searchHotels(input, env.RAKUTEN_APPLICATION_ID, env.RAKUTEN_ACCESS_KEY)
      if (hotels.length === 0) return '条件に合う宿が見つからなかったよ'
      return hotels.map((h, i) => 
        `${i + 1}. ${h.name}\n📍 ${h.address}\n💰 ${h.minCharge.toLocaleString()}円〜\n✨ ${h.special}\n⭐ ${h.reviewAverage}（${h.reviewCount}件）\n🔗 ${h.url}\n📋 ${h.planListUrl}`
      ).join('\n\n')
    }
